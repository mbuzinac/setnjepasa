...
if(isset($_GET["sifra"])){
	...
	$izraz=$veza->prepare("select count(vlasnik_psa) from pas where vlasnik_psa=:sifra");
	$izraz->execute($_GET);
	$ukupnoPas = $izraz->fetchColumn();
}
...
if(isset($_POST["obrisi"])){
		...
		unset($_POST["obrisi"]);
		$izraz=$veza->prepare("delete from vlasnik_psa 
		where sifra=:sifra");
		$izraz->execute($_POST);
		//print_r($_POST);
		//exit;
	...
	<body>
		...
		<div class="row">
			...
			<div class="large-6 columns">
				...
				<?php else:?>
					Vlasnik se ne može obrisati jer posjeduje psa
					<?php endif;?>
			</div>
		</div>
